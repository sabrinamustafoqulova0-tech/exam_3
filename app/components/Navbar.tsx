  ...
  useEffect(() => {
    const updateUnread = () => {
      if (typeof window !== "undefined") {
        const val = localStorage.getItem("total_unread_count");
        setUnreadCount(val ? parseInt(val, 10) || 0 : 0);
      }
    };

    updateUnread();

    window.addEventListener("unread-count-changed", updateUnread);
    window.addEventListener("storage", updateUnread);

    return () => {
      window.removeEventListener("unread-count-changed", updateUnread);
        ...
        >
          <button className="flex items-center gap-4 hover:bg-gray-100 p-3 rounded-xl">
            <div className="relative inline-flex">
              {pathname === "/messages" ? <ChatIcon /> : <ChatOutlinedIcon />}
              {unreadCount > 0 && (
                <span className="absolute -top-1.5 -right-1.5 bg-red-500 text-white rounded-full min-w-[16px] h-4 px-1 text-[10px] font-bold flex items-center justify-center border-2 border-white z-10 animate-pulse">
                  {unreadCount}
                </span>
              )}
            </div>
